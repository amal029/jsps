...
%Introducing wait constructs can change a behavior of SystemJ programs.
%Consider a snippet of code shown in Figure~\ref{fig:oo1} where the
%reaction R2 emits signal A, which is captured by R1, and as a result,
%signal O is emitted in the same tick. On the other hand,
...
%signal A. Such changes in the program behavior can be avoided if the
%\texttt{wait\_exact} does not consume logical ticks, i.e. if it is
%replaced with, for example, \texttt{Thread\-.sleep()} method in Java.
%However, this will also delay the execution of all other reactions in
%the same clock-domain since they execute in lock-step, i.e. they are
%synchronized at tick boundaries. As a consequence, WCRT of a
%clock-domain would be increased as much as the largest amount of time
%used in the \texttt{wait} construct. Our approach does not block entire
%clock-domain because our technique transforms the \texttt{wait}
%constructs into a logical times using the \texttt{pause} statement. In
%our opinion, this is more preferred solution in most cases, since
%increasing WCRT can negatively impact many time-critical applications.
%Furthermore, there already exists a verification approach that can
%formally verify the functional correctness of the SystemJ program
%\cite{7005124}. This approach can also be applied to the program using
%the \texttt{wait} constructs, since they are translated into the
%original SystemJ kernel statements.

The introduced wait construct is transformed into a sequence of SystemJ
...
%\subsection{Interaction of synchronous parallelism and wait constructs} 
